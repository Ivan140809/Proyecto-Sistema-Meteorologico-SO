...
while(true) {
  ...
  *nl = '\0';
  ...
  int idx = buscar_estacion(m.estacion);
  ...
  if (sem_wait(&estaciones[idx].buffer.espacios_libres) == -1) {
   perror("sem_wait espacios_libres");
   continue;
  }

  pthread_mutex_lock(&estaciones[idx].buffer.mutex_buffer);

  estaciones[idx].buffer.datos[estaciones[idx].buffer.in] = m;

  estaciones[idx].buffer.in = (estaciones[idx].buffer.in + 1) %estaciones[idx].buffer.tam;

  pthread_mutex_unlock(&estaciones[idx].buffer.mutex_buffer);

  if (sem_post(&estaciones[idx].buffer.items_disponibles) == -1) {
...
for (int i = 0; i < NumEstaciones; i++) {
 ...
 if (!buffer_init(&estaciones[i].buffer,tamBuffer)) {
...
for (int i = 0; i < NumEstaciones; i++) {
 buffer_destroy(&estaciones[i].buffer);
}
